...
// @require      https://cdnjs.cloudflare.com/ajax/libs/seedrandom/2.3.11/seedrandom.min.js
// @grant        none
// ==/UserScript==
...
CLIENT.on("message", function(message){
    ...
    if(message.nick != CLIENT.attributes.nick && message.flair){
        ...
        if(flair.parts.length || flair.color || flair.bg || flair.font){
            latest = flair
        }
    }
})
...
CLIENT.submit = function(message){
    ...
    if(parts.length != 0){
        Math.seedrandom(latest.original)
